...
según tipo {
  "numero" -> lorea($"{x} es un número")
  "texto"  -> lorea($"{x} es un texto")
  _        -> lorea("no sé qué wea es esto")
}
```

---

## v0.3 — Herramientas <Badge text="Planificado" variant="note" />
...
## v0.4 — Biblioteca estándar <Badge text="Planificado" variant="note" />
...
:::note
Esta hoja de ruta es una intención, no un contrato. El orden puede cambiar según lo que se aprenda escribiendo programas reales en WN++. Si una funcionalidad de v0.3 resulta ser más urgente que una de v0.2, se mueve.
:::
