...
cachai (ingredientes == 0) {
  ...
  cachai (ingredientes < 3) {
    lorea("Alcanza para un completo no más")
  } si no {
...
cachai (tiene_palta y tiene_tomate) {
  lorea("Completo italiano asegurado")
}

cachai (tiene_mayo o tiene_palta) {
  lorea("Algo rico sale igual")
}

cachai (no tiene_mayo) {
  lorea("Sin mayo, más sano pero menos rico")
}
```
...
mientras (verdad) {  // bucle infinito a propósito
  ...
  lorea("Completo número " + completos_comidos)

  cachai (completos_comidos >= 3) {
    lorea("Ya, basta. Me voy a morir.")
    cortala
  }
}
// → Completo número 1
...
para (postre en postres) {
  cachai (postre == "mote con huesillos") {
    lorea("¡Este es! Me quedo con el " + postre)
    cortala
  }
  lorea("Pasando el " + postre + "...")
}
// → Pasando el manjar...
...
para (sabor en pedido) {
  cachai (sabor == "camarón") {
    lorea("Sin camarones, se nos acabaron")
    sigue
  }
  ...
  total = total + precio
  lorea("+ Empanada de " + sabor + ": $" + precio)
}

lorea("---")
lorea("Empanadas de pino: " + cant_pino)
lorea("Total: $" + total)
```
